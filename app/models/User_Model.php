...
class User_Model extends Query
{
    ...
    {
        return $this->table('users')->where('username', '=', $username)->where('role', '=', $role)->first();
    }

    public function MOD_GET_QUESTIONS_BY_USER_ID($user_id): array
    {
        return $this->table('security_questions')->where('user_id', '=', $user_id)->get();
    }
    ...
    public function MOD_RESET_PASSWORD(int $user_id, string $password): bool
    {
        ...
        return $this->table('users')->where('id', '=', $user_id)->update(['password_hash' => $password_hash]);
    }
    ...
    {
        return $this->table('users')->where('id', $user_id)->first();
    }

    public function MOD_GET_ALL_USERS_EXCLUDE_CURRENT_USER($user_id): array
    {
        return $this->table('users')->where('id', '!=', $user_id)->orderBy('id', 'DESC')->get();
    }

    public function MOD_GET_USERS_PAGINATED($exclude_user_id, int $limit, int $offset): array
    {
        return $this->table('users')->where('id', '!=', $exclude_user_id)->orderBy('id', 'DESC')->limit($limit, $offset)->get();
    }

    public function MOD_GET_USERS_COUNT($exclude_user_id): int
    {
        $users = $this->table('users')->where('id', '!=', $exclude_user_id)->get();
    ...
    public function MOD_SEARCH_USERS(string $search_input, string $role, string $status, int $exclude_user_id): array
    {
        ...
        if ($status !== '') $query->where('is_active', '=', (int)$status);
    ...
    {
        $defaultAnswerHash = '$2a$12$tz2Eh3rDTv2HxGrpayprcuZLLR7nw5neAUNZmXOLrm2SiFsk./Dhi';
