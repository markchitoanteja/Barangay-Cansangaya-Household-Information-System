...
class Router
{
    ...
    {
        // Clear any output already produced (prevents "floating" text)
        ...
        // Expose variables to error view
        $vars = compact('code', 'title', 'message', 'details', 'homeUrl');
        extract($vars, EXTR_SKIP);

        // Buffer + catch view errors
        ob_start();
        ...
        try {
            ...
            // Last-resort plain output (cannot rely on views anymore)
            echo "{$code} {$title} - {$message}";
